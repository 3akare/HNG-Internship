...
exports.register = async (req, res) => {
    ...
    if (!firstName || !lastName || !email || !password) {
        return res.status(422).json({ errors: [{ field: "all", message: "All fields are required" }] });
    }
    ...
    const organization = await Organization.create({ orgId, name: `${firstName}'s Organization` })
